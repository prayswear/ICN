...
def data_handler(data, address):
    ...
    if not packet_id in temp_packet_dict.keys():
        if packet_seq > top_n:
            ...
            temp_packet_dict[packet_id] = {}
            temp_packet_dict[packet_id]['icn_head'] = p
            temp_packet_dict[packet_id]['data'] = binascii.a2b_hex('00' * packet_length)
            temp_packet_dict[packet_id]['data'] = temp_packet_dict[packet_id][
            ...
            if packet_num == int(packet_num):
                temp_packet_dict[packet_id]['count'] = int(packet_num) * recv_times
            else:
                temp_packet_dict[packet_id]['count'] = (int(packet_num) + 1) * recv_times
            temp_packet_dict[packet_id]['total'] = temp_packet_dict[packet_id]['count']
